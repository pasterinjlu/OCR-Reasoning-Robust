...
def batch_inference(llm, data, img_dir, cot=False):
    ...
    for idx, out in zip(valid_idx, outputs):
        ...
        full_response = text  # CoT full response (after stripping <think>)
        ...
        if cot:
            for marker in ["The final answer is:", "the final answer is:", "Final answer:", "final answer:", "The answer is", "the answer is", "Answer:"]:
                if marker in text:
                    ...
                    break
        pred_map[idx] = (text, full_response)
...
def score_batch(batch_results, use_judge=True):
    ...
    for item, pred_tuple in batch_results:
        if pred_tuple is None:
            continue
        pred, full_response = pred_tuple
        ...
        if result is not None:
            if result:
                correct += 1
            total += 1
        details.append({"question": item["question"], "gt": gt, "pred": pred, "full_response": full_response, "correct": bool(result) if result is not None else None})
